...
polarisParamsMap.set('CUSTOM_HEADER', 'CUSTOM_HEADER')  // Added CUSTOM_HEADER
...
export function setAllMocks() {
  ...
  jest.spyOn(toolCacheLocal, 'downloadTool').mockResolvedValue(__dirname)
  jest.spyOn(io, 'rmRF').mockResolvedValue()
  jest.spyOn(toolCache, 'extractZip').mockResolvedValue('Extracted')
  jest.spyOn(utility, 'validateBridgeUrl').mockReturnValue(true)
