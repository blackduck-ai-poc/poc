...
import {GITHUB_ENVIRONMENT_VARIABLES, NON_RETRY_HTTP_CODES, RETRY_COUNT, RETRY_DELAY_IN_MILLISECONDS, BRIDGE_CLI_DEFAULT_PATH_LINUX_UPDATED, BRIDGE_CLI_DEFAULT_PATH_MAC, BRIDGE_CLI_DEFAULT_PATH_WINDOWS, MAC_PLATFORM_NAME, LINUX_PLATFORM_NAME, WINDOWS_PLATFORM_NAME} from '../application-constants'
...
export class Bridge {
  ...
  private getBridgeDefaultPath(): string {
    ...
    if (osName === MAC_PLATFORM_NAME) {
      ...
      bridgeDefaultPath = path.join(process.env['HOME'] as string, BRIDGE_CLI_DEFAULT_PATH_LINUX_UPDATED, subFolder)
  ...
  private getBridgeCLIDownloadDefaultPath(): string {
    ...
    if (osName === MAC_PLATFORM_NAME) {
      ...
      bridgeCLIDefaultPath = path.join(process.env['HOME'] as string, BRIDGE_CLI_DEFAULT_PATH_LINUX_UPDATED)
    } else if (osName === WINDOWS_PLATFORM_NAME) {
  ...
  async executeBridgeCommand(bridgeCommand: string, workingDirectory: string): Promise<number> {
    ...
    if (osName === MAC_PLATFORM_NAME || osName === LINUX_PLATFORM_NAME || osName === WINDOWS_PLATFORM_NAME) {
      ...
      try {
        return await exec(this.bridgeExecutablePath.concat(' ', bridgeCommand), [], execOp)
      } catch (errorObject) {
        throw errorObject
      }
    }
  ...
  async downloadBridge(tempDir: string): Promise<void> {
    try {
      ...
      if (!(await this.checkIfBridgeExists(bridgeVersion))) {
        ...
        // Clear the existing bridge, if avaiable so we will not have duplicate or extra bridge folder
        ...
        const sourceFile = extractZippedFilePath
          ...
          .concat(downloadResponse.filePath.split(pathSeparator).pop() as string)
          .split('.zip')[0]
